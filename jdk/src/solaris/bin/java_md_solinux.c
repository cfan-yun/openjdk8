...
LoadJavaVM(const char *jvmpath, InvocationFunctions *ifn)
{
    ...
    libjvm = dlopen(jvmpath, RTLD_NOW + RTLD_GLOBAL);//加载livjvm.so库文件，目的是验证一下三个方法，后面要用
    if (libjvm == NULL) {
    ...
    ifn->CreateJavaVM = (CreateJavaVM_t)
        dlsym(libjvm, "JNI_CreateJavaVM");//主要验证以下3大方法的可用性。--JNI_CreateJavaVM
    ...
    ifn->GetDefaultJavaVMInitArgs = (GetDefaultJavaVMInitArgs_t)
        dlsym(libjvm, "JNI_GetDefaultJavaVMInitArgs");//方法具体在jvm.cpp中。 --JNI_GetDefaultJavaVMInitArgs
    ...
    ifn->GetCreatedJavaVMs = (GetCreatedJavaVMs_t)
        dlsym(libjvm, "JNI_GetCreatedJavaVMs");//编译时都打包进libjvm.so库文件中了。--JNI_GetCreatedJavaVMs
...
/*//阻塞当前线程并开启一个新线程继续执行，主要通过操作系统函数pthread_create和pthread_join实现线程的创建和阻塞
 * Block current thread and continue execution in a new thread
 */
int
ContinueInNewThread0(int (JNICALL *continuation)(void *), jlong stack_size, void * args) {//JNAICALL 会回调
    int rslt;
#ifdef __linux__
    pthread_t tid;
    pthread_attr_t attr;
    ...
    if (pthread_create(&tid, &attr, (void *(*)(void*))continuation, (void*)args) == 0) {//创建线程
      void * tmp;
      pthread_join(tid, &tmp); //join 阻塞主线程，等待pthread_create创建的线程执行完之后，主线程继续执行。
      rslt = (int)tmp;
    } else {
     /*
