...
)
{
    ...
    if (JLI_IsTraceLauncher()) {
        ...
        AddOption("-Dsun.java.launcher.diag=true", NULL);//整理一些参数
    }
     ...
     *     bugid 5030265 below.)
     */
    SelectVersion(argc, argv, &main_class);//从jar包中的mainfest 文件或者命令行读取永华使用的jdk版本，判断当前版本是否合适

    CreateExecutionEnvironment(&argc, &argv,
                               jrepath, sizeof(jrepath),
                               jvmpath, sizeof(jvmpath),
                               jvmcfg,  sizeof(jvmcfg));//建立执行环境，比如看看jre有没有安装，读一下jre的路径

    ifn.CreateJavaVM = 0;
    ifn.GetDefaultJavaVMInitArgs = 0;//计时器之类的

    if (JLI_IsTraceLauncher()) {
        start = CounterGet();         //加载虚拟机开始计时
    }

    if (!LoadJavaVM(jvmpath, &ifn)) { //加载虚拟机（主要加载libjvm.so库文件并验证相关方法是否可用）
        return(6);
    }

    if (JLI_IsTraceLauncher()) {
        end   = CounterGet();        //加载虚拟机结束计时
    }
    ...
    if (IsJavaArgs()) {
        ...
        cpath = getenv("CLASSPATH");//设置classpath 从环境变量中获取
        if (cpath == NULL) {
            cpath = ".";
        }
        SetClassPath(cpath);
     ...
     */
    if (!ParseArguments(&argc, &argv, &mode, &what, &ret, jrepath))// 解析命令行参数 如-version ， -help等参数
    {
        return(ret);
    }

    /* Override class path if -jar flag was specified */
    if (mode == LM_JAR) {//对 -jar参数做特殊处理，解析-jar命令
        SetClassPath(what);     /* Override class path */
    }

    /* set the -Dsun.java.command pseudo property */
    SetJavaCommandLineProp(what, argc, argv);//解析 -Dsun.java.command=的命令行参数

    /* Set the -Dsun.java.launcher pseudo property */
    SetJavaLauncherProp();//解析 -Dsun.java.launcher=的命令行参数

    /* set the -Dsun.java.launcher.* platform properties */
    SetJavaLauncherPlatformProps();//解析 --Dsun.java.launcher.*的命令行参数

    return JVMInit(&ifn, threadStackSize, argc, argv, mode, what, ret); //通过JVMInit->ContinueInNewThread ->ContinueInNewThread0->JavaMain->InitializeJVM
}
/*
...
JavaMain(void * _args)
{
     ...
     */
    appClass = GetApplicationClass(env);//javaFX 没有MainClass而是通过ApplicationClass启动的，这里获取的是ApplicationClass
    ...
    mainID = (*env)->GetStaticMethodID(env, mainClass, "main",
                                       "([Ljava/lang/String;)V"); //获取Main方法的ID
    ...
    LEAVE();//main方法执行完毕，jvm退出，包含两步(*vm)->DetachCurrentThread, 让当前Main线程同时启动线程断联，然后创建一个新的名为DestroyJavaVM的线程，让该线程等待所有用户线程退出，最后执行(*vm)->DestroyJavaVM方法，销毁虚拟机
}
...
InitializeJVM(JavaVM **pvm, JNIEnv **penv, InvocationFunctions *ifn)
{
    ...
    r = ifn->CreateJavaVM(pvm, (void **)penv, &args);//这里开始创建虚拟机调用的是jni.cpp文件中的JNI_CreateJava
    JLI_MemFree(options);
    return r == JNI_OK;
}
                    ...
                    int mode, char *what, int ret)
{
    ...
    { /* Create a new thread to create JVM and invoke main method */
      ...
      rslt = ContinueInNewThread0(JavaMain, threadStackSize, (void*)&args);//创建一个新线程来创建虚拟机并触发javaMain方法（JNICALL 回调javaMain）
