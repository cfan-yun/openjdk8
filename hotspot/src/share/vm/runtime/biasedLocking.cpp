...
BiasedLocking::Condition BiasedLocking::revoke_and_rebias(Handle obj, bool attempt_rebias, TRAPS) {
  assert(!SafepointSynchronize::is_at_safepoint(), "must not be called while at safepoint");//1.必须在安全点

  // We can revoke the biases of anonymously-biased objects
  // efficiently enough that we should not cause these revocations to
  // update the heuristics because doing so may cause unwanted bulk
  // revocations (which are expensive) to occur.
  markOop mark = obj->mark();                                 //2.获取对象头的MarkWord
  if (mark->is_biased_anonymously() && !attempt_rebias) {     //3.没有线程获取偏向锁（线程ID为空），说明不处于偏向锁状态
    ...
    markOop unbiased_prototype = markOopDesc::prototype()->set_age(mark->age());//prototype本身构建的是 markOopDesc
    markOop res_mark = (markOop) Atomic::cmpxchg_ptr(unbiased_prototype, obj->mark_addr(), mark);//执行cas修改标记
    if (res_mark == biased_value) {//如果之前的和现在的一样，说明撤销成功了，BIAS_REVOKED本身是一个枚举
      return BIAS_REVOKED;
    }
  } else if (mark->has_bias_pattern()) {//4.已经偏向了
    Klass* k = obj->klass();//已经被线程偏向了，获取klass对象，即类本身的头，obj是他的实例
    ...
    if (!prototype_header->has_bias_pattern()) {
      ...
      // with it.                                           //对象当前的偏向状态已经过期，并且是不可偏向的，直接设置成已经撤销
      markOop biased_value       = mark;
      markOop res_mark = (markOop) Atomic::cmpxchg_ptr(prototype_header, obj->mark_addr(), mark);
      assert(!(*(obj->mark_addr()))->has_bias_pattern(), "even if we raced, should still be revoked");
      return BIAS_REVOKED;
    } else if (prototype_header->bias_epoch() != mark->bias_epoch()) {
      // The epoch of this biasing has expired indicating that the      //实例的epoch和类本身的epoch值不一样，说明
      // object is effectively unbiased. Depending on whether we need   //也就是说这个对象当前处于未偏向状态但是可偏向
      ...
      if (attempt_rebias) {
        assert(THREAD->is_Java_thread(), ""); //执行rebias ,wait 希望直接撤销
        markOop biased_value       = mark;
