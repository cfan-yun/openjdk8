...
void Thread::set_priority(Thread* thread, ThreadPriority priority) {
  ...
  // Can return an error!
  (void)os::set_priority(thread, priority);//设置优先级
}
...
void Thread::start(Thread* thread) {
  ...
  if (!DisableStartThread) {
    if (thread->is_Java_thread()) {//如果是Java线程，设置状态为RUNNABLE
...
static oop create_initial_thread(Handle thread_group, JavaThread* thread, TRAPS) {
  Klass* k = SystemDictionary::resolve_or_fail(vmSymbols::java_lang_Thread(), true, CHECK_NULL); //加载并初始化java.lang.Thread类
  instanceKlassHandle klass (THREAD, k);
  instanceHandle thread_oop = klass->allocate_instance_handle(CHECK_NULL);//分配一个instanceHandle

  java_lang_Thread::set_thread(thread_oop(), thread);//thread 实例opp对象保存关联的JavaThread对象
  java_lang_Thread::set_priority(thread_oop(), NormPriority);//设置线程优先级为NormPriority
  thread->set_threadObj(thread_oop());//JavaThread保存关联的Thread实例opp

  Handle string = java_lang_String::create_from_str("main", CHECK_NULL);//初始化一个字符串main,给线程取了个名字main
  ...
  JavaCalls::call_special(&result, thread_oop,
                                   ...
                                   thread_group,
                                   string,
                                   CHECK_NULL);//c++层面调用Java中Thread的构造方法，创建的Thread实例保存在thread_oop中
  return thread_oop();
}
...
JavaThread::JavaThread(ThreadFunction entry_point, size_t stack_sz) :
  Thread()//Thread 的构造方法就是初始化Thread的各属性
#if INCLUDE_ALL_GCS
  , _satb_mark_queue(&_satb_mark_queue_set),
  _dirty_card_queue(&_dirty_card_queue_set)
#endif // INCLUDE_ALL_GCS
{
  ...
  initialize();//初始化JavaThread自有的各种属性
  _jni_attach_state = _not_attaching_via_jni;
  set_entry_point(entry_point); //这里注意：run方法回调的入口点在这里设置进去了
  // Create the native thread itself.
  // %note runtime_23
  os::ThreadType thr_type = os::java_thread;
  thr_type = entry_point == &compiler_thread_entry ? os::compiler_thread : //是否编译线程，编译线程也是JavaThread,但是对应的os::ThreadType类型不同
                                                     os::java_thread;
  os::create_thread(this, thr_type, stack_sz);  //Java中的Thread对应的内核线程，原生线程，底层通过pthread_create 创建,创建成功后将其设置到Thread的_osthread属性中
  _safepoint_visible = false;                   //初始化结束后在startThread_lock上等待被唤醒，如果内存不足导致创建失败，则该属性为NULL
...
void JavaThread::run() {
  ...
  this->record_base_of_stack_pointer();//记录栈基地址

  // Record real stack base and size.
  this->record_stack_base_and_size();//初始化OSThread的栈帧

  // Initialize thread local storage; set before calling MutexLocker
  this->initialize_thread_local_storage(); //初始化ThreadLocalStorage

  this->create_stack_guard_pages();//创建stack_guard_pages

  this->cache_global_variables();//空实现

  // Thread is now sufficient initialized to be handled by the safepoint code as being
  // in the VM. Change thread state from _thread_new to _thread_in_vm
  ThreadStateTransition::transition_and_fence(this, _thread_new, _thread_in_vm);//修改线程状态，注意此时会检查是狗进入
  ...
  thread_main_inner(); //这个方法很重要，会调线程入口，执行具体的run方法并在执行完成后销毁当前实例
...
void JavaThread::thread_main_inner() {
  ...
  if (!this->has_pending_exception() &&
    ...
    HandleMark hm(this);
    this->entry_point()(this, this); //回调Java线程run方法的入口，执行entry_point函数，就是执行Thread的run方法
  }
...
void JavaThread::exit(bool destroy_vm, ExitType exit_type) {
    ...
    if (uncaught_exception.not_null()) {
      Handle group(this, java_lang_Thread::threadGroup(threadObj()));
      {
        ...
        LinkResolver::resolve_virtual_call(callinfo, threadObj, recvrKlass, thread_klass,
                                           vmSymbols::dispatchUncaughtException_name(),//获取到 dispatchUncaughtException 方法
                                           vmSymbols::throwable_void_signature(),
                                           KlassHandle(), false, false, THREAD);//获取一个方法集合的数据结构
        CLEAR_PENDING_EXCEPTION;
        methodHandle method = callinfo.selected_method();//找到 dispatchUncaughtException 方法
        if (method.not_null()) {//如果 dispatchUncaughtException 方法不为空
          JavaValue result(T_VOID);
          JavaCalls::call_virtual(&result,
                                  ...
                                  vmSymbols::throwable_void_signature(),
                                  uncaught_exception,
                                  THREAD);//jvm处理异常实际是在这个地方触发，然后对异常进行打印
        } else {//如果为空
          KlassHandle thread_group(THREAD, SystemDictionary::ThreadGroup_klass());//如果是空，调用线程组的处理异常的方法
          JavaValue result(T_VOID);
          JavaCalls::call_virtual(&result,
                                  group, thread_group,
  ...
  Threads::remove(this);//这里将执行完成的线程移除活动线程链表 满足条件调用Threads::destroy_vm
}
...
jint Threads::create_vm(JavaVMInitArgs* args, bool* canTryAgain) {
  ...
  extern void JDK_Version_init(); //预初始化，可以由特定的平台按需覆盖定义，默认为空实现

  // Check version //检查当前JDK版本是否支持代码编译的JDK版本
  if (!is_supported_jni_version(args->version)) return JNI_EVERSION;

  // Initialize the output stream module
  ostream_init();//流模块的初始化

  // Process java launcher properties.
  Arguments::process_sun_java_launcher_properties(args);//处理启动器次昂管参数。如-Dsun.java.launcher

  // Initialize the os module before using TLS
  os::init();//OS模块的初始化，包含设置内存页大小，获取物理内存大小，锁的初始化

  // Initialize system properties.      //初始化系统属性，所有的属性通过Arguments类的静态属性_system_properties保存
  Arguments::init_system_properties();  //每个属性对应一个SystemProperty实例，同故宫链表的形式关联起来

  // So that JDK version can be used as a discrimintor when parsing arguments
  JDK_Version_init();//初始化JDK_Version类的静态变量_current, 获取当前JDK的版本信息，如_major主版本号，_minor此版本号等

  // Update/Initialize System properties after JDK version number is known
  Arguments::init_version_specific_system_properties();//初始特定JDK版本的系统属性

  // Parse arguments
  jint parse_result = Arguments::parse(args);//解析命令行参数
  if (parse_result != JNI_OK) return parse_result;

  os::init_before_ergo();//初始化可用CPU核数，根据配置设置大内存页

  jint ergo_result = Arguments::apply_ergo();//设置系统参数，如是否使用指针压缩，堆内存大小，GC参数等
  ...
  TraceVmCreationTime create_vm_timer;//记录启动时间
  create_vm_timer.start();

  // Timing (must come after argument parsing)
  TraceTime timer("Create VM", TraceStartupTime);//打印启动时间

  // Initialize the os module after parsing the args //os模块的二次初始化，包含线程锁的初始化，polling_page和mem_s...
  jint os_init_2_result = os::init_2(); //设置线程锁，线程优先级策略的初始化
  if (os_init_2_result != JNI_OK) return os_init_2_result;

  jint adjust_after_os_result = Arguments::adjust_after_os();//UseNUMA时，额外的参数配置
  ...
  ostream_init_log();//初始化GC日志和LoadedClass日志的FileStream对象

  // Convert -Xrun to -agentlib: if there is no JVM_OnLoad //将参数 -Xrun 转换成 -agentlib参数
  ...
  if (Arguments::init_agents_at_startup()) {//根据-agentlib/-agentpanth and 转换后  -Xrun 参数创建agents
  ...
  vm_init_globals();//初始化Events,各种锁，ChunkPool, PerfMemory

  // Attach the main thread to this os thread //创建一个新的JavaThread 并设置相关属性，注意这里并未创建一个新的线程
  JavaThread* main_thread = new JavaThread(); //main线程在这里创建的，但是是如何与内核线程对应呢？？
  ...
  if (!main_thread->set_as_starting_thread()) {//该方法内部实现关联javaMain线程与os线程对应
  ...
  ObjectMonitor::Initialize() ;//ObjectMonitor初始化，锁初始化，synchronized

  // Second phase of bootstrapping, VM is about entering multi-thread mode
  MemTracker::bootstrap_multi_thread();

  // Initialize global modules
  jint status = init_globals();//全局模块初始化，如management模块，Bytecodes模块，classloader模块，CodeCache模块
  if (status != JNI_OK) {
    delete main_thread;
    *canTryAgain = false; // don't let caller call JNI_CreateJavaVM again
    return status;
  ...
  JvmtiExport::transition_pending_onload_raw_monitors();//将加载时创建的raw monitor 转换为真正的raw monitor

  // Fully start NMT
  MemTracker::start();

  // Create the VMThread //创建VM线程，比如垃圾回收相关的线程
  ...
  if (VerifyDuringStartup) {
    ...
    VM_Verify verify_op;
    VMThread::execute(&verify_op); //验证VMThread状态
  }
  ...
  JvmtiExport::enter_start_phase();//标记JVMTI启用

  // Notify JVMTI agents that VM has started (JNI is up) - nop if no agents.
  JvmtiExport::post_vm_start();//通知JVMTI agents 虚拟机已经启动
  ...
  {//加载核心Java类文件
    ...
    main_thread->set_threadObj(thread_object);//将main_thread 同Java中的thread_object关联起来
    ...
    call_initializeSystemClass(CHECK_0); //调用Java.lang.System中的initializeSystemClass方法初始化系统属性
...
bool Threads::destroy_vm() {
  ...
  _vm_complete = false;
#endif
  // Wait until we are the last non-daemon thread to execute//等待最后一个非守护线程执行完成条件
  { MutexLocker nu(Threads_lock);
    while (Threads::number_of_non_daemon_threads() > 1 )//非守护线程大于1，则一直等待(知道等于1的时候往下执行，即只剩下自己DestroyVM线程)
      // This wait should make safepoint checks, wait without a timeout,
      // and wait as a suspend-equivalent condition.
      //
      ...
      Threads_lock->wait(!Mutex::_no_safepoint_check_flag, 0,
                         Mutex::_as_suspend_equivalent_flag);//这里会阻塞的线程会被线程执行完毕后，从链表移除的JavaThread::exit -> Thread::remove方法里的notify唤醒
  }
  ...
  if (JDK_Version::is_jdk12x_version()) {
    ...
    thread->invoke_shutdown_hooks();//java的钩子函数在这里触发Runtime.getRuntime().addShutdownHook();
  }
...
void Threads::remove(JavaThread* p) {
  ...
  { MutexLocker ml(Threads_lock);
    ...
    while (current != p) {//线程以链表的形式存在，判断当前线程是否要删除的线程，如果不是，后移，知道找到为止
      prev    = current;
      current = current->next();
    }

    if (prev) {//被删除线程是否有前驱节点，如果有，则直接删除，让前驱节点指向被删除的节点的next节点（跨过被删除的节点）
      prev->set_next(current->next());
    } else {//如果没有前驱节点，说明删除的节点是第一个节点，直接_thread_list指向它的next节点
      _thread_list = p->next();
    }
    ...
    if (threadObj == NULL || !java_lang_Thread::is_daemon(threadObj)) {
      _number_of_non_daemon_threads--;//非守护线程数量减一
      daemon = false;

      // Only one thread left, do a notify on the Threads_lock so a thread waiting
      // on destroy_vm will wake up.
      if (number_of_non_daemon_threads() == 1)//当非守护线程数量为1时，唤醒在Threads::destroy_vm方法等待的线程（其实最后那个非守护线程）
