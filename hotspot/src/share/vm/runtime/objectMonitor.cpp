...
void ATTR ObjectMonitor::enter(TRAPS) {//重量级锁入口
  // The following code is ordered to check the most common cases first
  // and to reduce RTS->RTO cache line upgrades on SPARC and IA32 processors.
  Thread * const Self = THREAD ;
  void * cur ;

  cur = Atomic::cmpxchg_ptr (Self, &_owner, NULL) ;//通过cas设置owner为当前线程，设置成功表示获取到锁
  if (cur == NULL) {//修改成功，返回（如果为NULL,则修改为最新值，并且返回原值，否则，原值不是null，不做更新，并返回新值）
  ...
  if (cur == Self) {//如果是重入的，则_recursions+1 （cas 没做更新，直接返回最新值）
     // TODO-FIXME: check for integer overflow!  BUGID 6557169.
     _recursions ++ ;
     return ;
  }

  if (Self->is_lock_owned ((address)cur)) {//从轻量级锁膨胀来的（本身就有锁，再次进来获取锁，重入次数就是1了）
    assert (_recursions == 0, "internal state error");
    _recursions = 1 ;
  ...
  // We've encountered genuine contention.//到了这里，表示真正的遇到了锁竞争。开始真正处理竞争情况
  ...
  if (Knob_SpinEarly && TrySpin (Self) > 0) {//自旋操作，默认是10次
     assert (_owner == Self      , "invariant") ;
     assert (_recursions == 0    , "invariant") ;
...
void ATTR ObjectMonitor::EnterI (TRAPS) {
    ...
    node.TState  = ObjectWaiter::TS_CXQ ;//设置下状态为TS_CXQ

    // Push "Self" onto the front of the _cxq.//这里其实是塞到头部（后进先出）
    // Once on cxq/EntryList, Self stays on-queue until it acquires the lock.
    // Note that spinning tends to reduce the rate at which threads
    // enqueue and dequeue on EntryList|cxq.
    ObjectWaiter * nxt ;
    for (;;) {//循环，保证将node插入队列
        node._next = nxt = _cxq ;//将node插入到cxq的头部
        if (Atomic::cmpxchg_ptr (&node, &_cxq, nxt) == nxt) break ; //这里可能会有多个线程争抢塞入头节点，cas修改，修改成功则返回

        // Interference - the CAS failed because _cxq changed.  Just retry.
        // As an optional optimization we retry the lock.
        if (TryLock (Self) > 0) {// 默默的tryLock一下，真的不想挂起啊！！！如果成功return,此时不进入队列了
            assert (_succ != Self         , "invariant") ;
            assert (_owner == Self        , "invariant") ;
            assert (_Responsible != Self  , "invariant") ;
            return ;
    ...
    for (;;) {//这里开始真正的阻塞
        ...
        if (TryLock (Self) > 0) break ; //自旋，临死之前我再trylock一下
        assert (_owner != Self, "invariant") ;
        ...
        if (_Responsible == Self || (SyncFlags & 1)) {
            ...
            Self->_ParkEvent->park ((jlong) RecheckInterval) ;//park 底层会调用mutex (进入内核态)
            // Increase the RecheckInterval, but clamp the value.
            RecheckInterval *= 8 ;
            if (RecheckInterval > 1000) RecheckInterval = 1000 ;
        } else {
            TEVENT (Inflated enter - park UNTIMED) ;
            Self->_ParkEvent->park() ;//park底层会调用mutex(进入内核态)，终于挂起了，线程卡死在这里，等待唤醒
        }

        if (TryLock(Self) > 0) break ;//这段注意，此时是唤醒之后，被唤醒之后，依然需要抢锁
