...
void ObjectSynchronizer::fast_enter(Handle obj, BasicLock* lock, bool attempt_rebias, TRAPS) {//obj
 if (UseBiasedLocking) {//检查是否启用了偏向锁，如果没有启用，直接执行slow_enter
    if (!SafepointSynchronize::is_at_safepoint()) {//如果不在安全点，(stop-the-World暂停机制，Java代码会挂起)，stw,即没有字节码在执行，整个虚拟机处于一个暂停态
      BiasedLocking::Condition cond = BiasedLocking::revoke_and_rebias(obj, attempt_rebias, THREAD);//获取偏向锁
      if (cond == BiasedLocking::BIAS_REVOKED_AND_REBIASED) {//这里就是判断偏向锁标志位1，锁标记为01，如果不是，
        return;//撤销完返回
      }
    } else {//无法偏向，则在安全点撤销偏向锁
      assert(!attempt_rebias, "can not rebias toward VM thread");
      BiasedLocking::revoke_at_safepoint(obj);//在安全点撤销偏向锁(移除偏向锁)
    }
    assert(!obj->mark()->has_bias_pattern(), "biases should be revoked by now");
 }//以上步骤撤销偏向锁，偏向锁撤销掉才能走轻量级锁

 slow_enter (obj, lock, THREAD) ;//走轻量级锁模式
}
...
void ObjectSynchronizer::slow_enter(Handle obj, BasicLock* lock, TRAPS) {//获取轻量级锁的入口
  markOop mark = obj->mark();
  assert(!mark->has_bias_pattern(), "should not see bias pattern here");//此时不应该是偏向锁

  if (mark->is_neutral()) {//如果此时是无锁状态
    // Anticipate successful CAS -- the ST of the displaced mark must
    // be visible <= the ST performed by the CAS.
    lock->set_displaced_header(mark);//设置头信息：虚拟机首先将在当前线程的栈帧中建立一个名为锁记录（Lock Record）的对象
    if (mark == (markOop) Atomic::cmpxchg_ptr(lock, obj()->mark_addr(), mark)) {//如果是没有锁的，就把线程的
      TEVENT (slow_enter: release stacklock) ;//将lock作为原来的对象头，因为BasicLock本身的大小就是8个字节的，
      return ;                                //如果设置成功表示已经获取到轻量级锁，返回。
    }
    // Fall through to inflate() ... //这里cas获取轻量级锁失败后直接膨胀进入258行
  } else //如果对象已经被锁了，看一下是不是请求锁的线程锁的，如果是就万事大吉
  if (mark->has_locker() && THREAD->is_lock_owned((address)mark->locker())) {
    assert(lock != mark->locker(), "must not re-lock the same lock");
    assert(lock != (BasicLock*)obj->mark(), "don't relock with same BasicLock");
    lock->set_displaced_header(NULL);//清除对象头信息（已经分配了对应的basicLock，这个BasicLock没有用了）
    return;
  }
  ...
  lock->set_displaced_header(markOopDesc::unused_mark());//因为ObjectMonitor会保存对象的对象头，所以此处不
  ObjectSynchronizer::inflate(THREAD, obj())->enter(THREAD);//如果还不是的话就要做锁膨胀了，真的Moniter就要出
}
...
ObjectMonitor * ATTR ObjectSynchronizer::inflate (Thread * Self, oop object) {
  ...
  for (;;) {
      ...
      // CASE: inflated
      if (mark->has_monitor()) {//已经膨胀完成了，直接退出
      ...
      if (mark == markOopDesc::INFLATING()) {//正在膨胀中，自旋等待
      ...
      if (mark->has_locker()) {//大量的轻量级锁状态的线程进行cas操作，修改状态为正在膨胀中，大量的轻量级锁，转换为重量级锁
