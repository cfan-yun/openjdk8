...
IRT_ENTRY_NO_ASYNC(void, InterpreterRuntime::monitorenter(JavaThread* thread, BasicObjectLock* elem))
  ...
  if (PrintBiasedLockingStatistics) {
    Atomic::inc(BiasedLocking::slow_path_entry_count_addr());//增加计数器
  }
  Handle h_obj(thread, elem->obj());//获取关联的对象
  assert(Universe::heap()->is_in_reserved_or_null(h_obj()),
         "must be NULL or an object");
  if (UseBiasedLocking) {//如果开启了偏向锁功能，1.6之后默认开启，如果确定线程有激烈竞争，可以直接取消偏向锁，使用
    // Retry fast entry if bias is revoked to avoid unnecessary inflation
    ObjectSynchronizer::fast_enter(h_obj, elem->lock(), true, CHECK);//走快速enter可以避免不必要膨胀为重量级锁
  } else {//当关闭偏向锁功能时，或多个线程竞争偏向锁，导致偏向锁升级为轻量级锁，会尝试获取轻量级锁，这是入口
...
//%note monitor_1 //锁释放就是目标对象的对象头恢复成无锁状态
