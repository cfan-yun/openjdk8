...
static void thread_entry(JavaThread* thread, TRAPS) {//这里就是反向调用Java中的Thread类的run方法入口
  HandleMark hm(THREAD);
  ...
  JavaCalls::call_virtual(&result,
                          ...
                          THREAD);//此处回调Java中的Thread类的run方法
}


JVM_ENTRY(void, JVM_StartThread(JNIEnv* env, jobject jthread))
  JVMWrapper("JVM_StartThread");//启动线程start0方法入口
  ...
  // in Thread::start.
  {
    ...
    if (java_lang_Thread::thread(JNIHandles::resolve_non_null(jthread)) != NULL) {
      throw_illegal_thread_state = true;//如果关联的javaThread不为空，说明线程已经启动过了，需要抛出异常
    } else {
      // We could also check the stillborn flag to see if this thread was already stopped, but
      // for historical reasons we let the thread detect that itself when it starts running

      jlong size =
             java_lang_Thread::stackSize(JNIHandles::resolve_non_null(jthread));//获取stackSize属性，若线程设置了stackSize则会获取到，否则直接返回0
      ...
      if (native_thread->osthread() != NULL) {//osthread 不为空，prepare方法会建立JAVA Thread 实例和JavaThread对象之间的关联关系
        // Note: the current thread is not being used within "prepare".
        native_thread->prepare(jthread);    //并将Java线程的优先级映射成原生线程的优先级
      }
    }
  }

  if (throw_illegal_thread_state) { //关联的JavaThread 不为空，抛出异常
    THROW(vmSymbols::java_lang_IllegalThreadStateException());
  }

  assert(native_thread != NULL, "Starting null thread?");

  if (native_thread->osthread() == NULL) {//创建osthread失败抛出异常
...
JVM_ENTRY(void, JVM_SetThreadPriority(JNIEnv* env, jobject jthread, jint prio))
  ...
  // Ensure that the C++ Thread and OSThread structures aren't freed before we operate
  MutexLocker ml(Threads_lock);//获取Threads_lock锁
  oop java_thread = JNIHandles::resolve_non_null(jthread);//获取关联的Thread实例oop
  java_lang_Thread::set_priority(java_thread, (ThreadPriority)prio);//设置Thread实例的priority属性
  JavaThread* thr = java_lang_Thread::thread(java_thread);//JavaThread实例
  if (thr != NULL) {                  // Thread not yet started; priority pushed down when it is
    Thread::set_priority(thr, (ThreadPriority)prio);//设置优先级
  }
JVM_END
