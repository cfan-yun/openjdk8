...
// Thread start routine for all newly created threads //所有线程创建完都会执行这个方法，而且会阻塞，知道线程状态为RUNNABLE
static void *java_start(Thread *thread) {
  ...
  // handshaking with parent thread
  {
    ...
    // notify parent thread
    osthread->set_state(INITIALIZED);//JAVA中INITIALIZED->NEW
    sync->notify_all();

    // wait until os::start_thread()
    while (osthread->get_state() == INITIALIZED) { //这里会循环阻塞，一直等到线程状态为RUNNABLE状态才往下执行
      sync->wait(Mutex::_no_safepoint_check_flag);
    }
  }

  // call one more level start routine
  thread->run(); //此时线程已经是RUNNABLE状态，于是执行run方法
...
bool os::create_thread(Thread* thread, ThreadType thr_type, size_t stack_size) {
  ...
  OSThread* osthread = new OSThread(NULL, NULL); //OSThread是java线程和内核线程关联的直接桥梁
  if (osthread == NULL) {
    return false;
  }
  ...
  thread->set_osthread(osthread);//先将osthread与javathread 关联
...
bool os::create_attached_thread(JavaThread* thread) {
  ...
  // Allocate the OSThread object
  OSThread* osthread = new OSThread(NULL, NULL);//Java对象和操作系统原生线程的中介对象，通过该对象进行关联
  ...
  // Store pthread info into the OSThread
  osthread->set_thread_id(os::Linux::gettid());//将当前内核线程的id等信息设置进osthread对象中
  ...
  osthread->set_state(RUNNABLE);//创建初始就是RUNNABL状态

  thread->set_osthread(osthread);//在此产生关联，真正将os内核线程和main线程关联的地方在这里
...
void os::pd_start_thread(Thread* thread) {
  ...
  Monitor* sync_with_child = osthread->startThread_lock(); //唤醒之前创建的在startThread_lock上等待的子线程，开始执行javaThread的run方法
  MutexLockerEx ml(sync_with_child, Mutex::_no_safepoint_check_flag);
...
OSReturn os::set_native_priority(Thread* thread, int newpri) {
  ...
  int ret = setpriority(PRIO_PROCESS, thread->osthread()->thread_id(), newpri);//操作系统函数，设置优先级
